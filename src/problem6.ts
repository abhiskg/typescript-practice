...
// Feet to Inch
function feetToInch(feet: number) {
  const inch = feet * 12;
  return inch;
}
...
// Centimeter to meter
function centimeterToMeter(centimeter: number) {
...
function paperRequirements(
  ...
  const totalRequiredPaper =
    firstCopy * firstPageNeed +
...
function bestFriend(name: string[]) {
...
function positiveArray(num: number[]) {
  const newArray = [];
