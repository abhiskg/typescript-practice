...
function oilPrice(dieselQty: number, petrolQty: number, octaneQty: number) {
  if (
...
function publicBusFare(totalMember: number) {
...
interface IsBestFriend {
  name: string;
  friend: string;
}
// Function to check if given two friends are best friend or not
function isBestFriend(firstFriend: IsBestFriend, secondFriend: IsBestFriend) {
